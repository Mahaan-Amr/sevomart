...
است. schema نسخهٔ یک manifest در `ops/demo/manifest.v1.json` باقی مانده و manifest اجرایی
نسخهٔ ۲ اکنون baseline کامل دادهٔ نمایشی را حمل می‌کند. factory سناریوهای QA همچنان مستقل
است و manifest کامل demo را مصرف نمی‌کند.
...
- اجرای apply، دادهٔ canonical، projectionهای demo، مالکیت resourceها، receipt نسخه manifest
  و report را اتمیک ثبت می‌کند. dry-run و اجرای تکراری شمار
  `created/updated/retired/unchanged` را بدون تغییر داده گزارش می‌کنند و فقط شناسه‌های پایدار
  namespace `sevo.demo` را همگرا می‌سازند؛ رکورد انسان خارج از این مالکیت هدف write نیست.

## baseline نسخهٔ ۲

- پنج هویت ورودپذیر با شماره‌های ساختگی `09000000001` تا `09000000005` و صفحهٔ شروع مناسب
  خریدار، فروشنده، متقاضی، عامل بررسی و مدیر دسترسی گزارش می‌شوند. سه مالک فروشگاه پس‌زمینه
  روش ورود نمایشی ندارند.
- چهار فروشگاه، یازده کالا، شش محتوای فروش، دو دنبال‌کردن، یک سبد و نشانی، سه گفت‌وگو و ده
  سفارش مصوب ساخته می‌شوند. یک تجربه خرید تأییدشده و یک پرونده اختلاف دارای مدارک مصنوعی و
  اقدام ممیزی‌شده عامل نیز وجود دارد.
- سفارش‌ها وضعیت‌های انتظار پرداخت، بررسی پرداخت، قابل اقدام، آماده‌سازی، ارسال، تحویل،
  انقضا، لغو در انتظار بازپرداخت، بازپرداخت‌شده و اختلاف باز را با snapshot و زمان نسبی
  سازگار می‌سازند.
- فایل‌های واقعی و پایدار برای تصویر کالا و محتوای فروش پیش از transaction در MinIO داخلی
  همگرا می‌شوند و سپس رکوردهای media و ارجاع‌های دامنه‌ای در همان transaction ثبت می‌شوند.
  حذف resource ابتدا دسترسی canonical و projection آن را می‌بندد و پس از commit فقط objectهای
  متعلق به همان namespace را پاک می‌کند؛ شکست حذف object داده را دوباره عمومی نمی‌کند.
- هر ماژول فقط جدول‌های متعلق به خودش را از adapter محلی `demo-seed` همگرا می‌کند. orchestrator
  مرکزی تنها ترتیب interfaceها، قفل، ownership عمومی resource و receipt را مدیریت می‌کند.
- projectionهای کشف، دنبال‌کردن و شمار عمومی پس از داده canonical در همان transaction از
  منابع اصلی بازسازی می‌شوند. هیچ بازدید، پسند یا سیگنال محبوبیت عمومی ساخته نمی‌شود.
- OTP توسعه در Compose فقط پنج شمارهٔ صریح بالا را می‌پذیرد. provider پرداخت توسعه نتیجه را
  فقط از query صریح `scenario=success|failure|pending` می‌گیرد و دیگر از UUID، مبلغ یا قاعده
  پنهان نتیجه‌گیری نمی‌کند. URL اولیه `success` را شفاف انتخاب می‌کند و برای بررسی دو مسیر
  دیگر می‌توان همان URL را با مقدار `failure` یا `pending` باز کرد.
