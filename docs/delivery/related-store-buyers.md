...
مرجع‌ها: [[ساخت] ایجاد query خریداران مرتبط و reveal زمینه‌مند](https://github.com/Mahaan-Amr/sevomart/issues/136)
و [[ساخت] افزودن قرارداد تاریخچه سفارش خریدار در همان فروشگاه](https://github.com/Mahaan-Amr/sevomart/issues/220)
...
- `GET /v1/seller/orders/{orderId}/buyer-orders` تاریخچه سفارش همان خریدار را با
  استفاده از سفارش زمینه‌ای پیدا می‌کند؛ شناسه خریدار از کلاینت پذیرفته نمی‌شود.
  نتیجه فقط به فروشگاه فعال فروشنده محدود است و در هر ردیف شناسه سفارش، وضعیت
  پرداخت، وضعیت اختیاری انجام و زمان ثبت را برمی‌گرداند. cursor امضاشده به فروشگاه
  و سفارش زمینه‌ای bind است، پاسخ `no-store` می‌ماند و داده حساس تازه‌ای ندارد.
...
- contract: شکل `ListStoreBuyers.v1`، masking، سقف صفحه، ورودی reveal و OpenAPI
  canonical، و شکل حداقلی `StoreBuyerOrderHistoryRead.v1` پوشش داده می‌شوند.
...
- PostgreSQL integration: جداسازی فروشگاه، جست‌وجوی نام/شماره سفارش، دو صفحه
  ...
  audit بدون PII و رد دسترسی میان‌فروشگاهی پوشش داده می‌شوند. تاریخچه نیز با دو
  صفحه، اتصال cursor به سفارش زمینه‌ای، نبود شناسه خریدار در پاسخ و رد سفارش
  فروشگاه دیگر آزموده می‌شود.
