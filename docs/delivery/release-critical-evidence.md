...
- `pnpm qa:evidence:e2e` کل زنجیره unit، contract، integration و E2E نامزد را دو بار با
  ...
  لایه مرورگر روی همان مقصد disposable با adapterهای واقعی PostgreSQL و MinIO اجرا
  می‌شود؛ storage حافظه‌ای فقط برای آزمون‌های درون‌پردازه باقی می‌ماند.
...
زیرساخت [[ساخت] ثبت baselineهای بصری و evidence pack نسخه نمایشی](https://github.com/Mahaan-Amr/sevomart/issues/166)
برای تحویل به دروازه نهایی آماده است. `captureReleaseCheckpoint` در حالت‌های موجود تصویر منتخب با پوشاندن
...
یافتهٔ `serious` یا `critical` چه قطعی باشد و چه unresolved/incomplete، checkpoint را رد می‌کند.
...
باقی می‌مانند. مقدار `browserActivity: null` یا نبودن guard اکنون خودِ run را رد می‌کند؛
فقط چهار شمارندهٔ صریح و صفر به‌عنوان مشاهدهٔ پاک پذیرفته می‌شوند. guard صفحه آغازین و هر
صفحه تازه در همان context را می‌بیند و فقط لغوهای دقیق ناوبری مرورگر را نادیده می‌گیرد.
فایل‌های موقت خامی که خود Playwright برای شکست می‌سازد، در پایان اجرا فقط پس از بررسی
مسیر ورودی و مقصد واقعی داخل پوشه همان run حذف می‌شوند؛ alias خارج از پوشه نیز رد می‌شود.
خطای پاک‌سازی، run را ناموفق می‌کند و مانع
...
پاسخ HTTP یا شکست شبکه‌ای عمدی فقط در آزمونی پذیرفته می‌شود که سناریوی مشخص آن را annotation
کرده باشد و method و خانواده route نیز دقیقاً مطابق قرارداد باشد. پروژه‌های Chromium نامزد
برای هر annotation فقط یک رخداد مطابق را مصرف می‌کنند؛ تکرار همان خطا دوباره run را رد می‌کند.
پروژه‌های Chromium نامزد فقط فایل‌های E2E ارجاع‌شده در manifest را اجرا می‌کنند؛ فایل بیرون
از ماتریس نمی‌تواند به‌طور تصادفی در گزارش نامزد وارد شود.

تولید دو اجرای کامل روی SHA پاک، بسته قابل نگه‌داری، تأیید دو توسعه‌دهنده و receipt نهایی
جزء اجرای [[ساخت] اجرای نهایی دروازه release-critical نسخه نمایشی](https://github.com/Mahaan-Amr/sevomart/issues/167)
است و این سند آن خروجی‌ها را از پیش تأیید نمی‌کند.
...
WebKit واقعی، از دست رفتن مقدار ورودی پیش از hydration صفحه ورود را نیز بازتولید کرد:
فیلد DOM پر بود ولی درخواست OTP مقدار خالی state را می‌فرستاد. کنترل شماره و ارسال تا
آماده‌شدن کلاینت غیرفعال‌اند. آزمون `login-hydration.spec.ts` با نگه‌داشتن بارگذاری
JavaScript، وضعیت قبل و بعد از hydration و ارسال مقدار واردشده را بررسی می‌کند و جزو
smoke هر دو اندازه WebKit است. این آزمون به تأخیر ثابت یا retry تکیه ندارد.
آزمون WebKit همچنین آشکار کرد که تنظیم پیش‌فرض موتور، لینک‌ها را وارد چرخه Tab نمی‌کند؛
پیوند بازگشت سفارش `tabIndex=0` صریح دارد تا دسترسی آن به تنظیم محلی مرورگر وابسته نباشد.
fixture سبد مهمان نیز تصویر واقعی را از مسیر API رسانه و MinIO می‌سازد؛ شناسه تصادفی بدون
asset دیگر نمی‌تواند 404 تصویر را پنهان کند. گالری‌های کالا و تجربه خرید landmark معنایی
دارند تا نام دسترس‌پذیر روی عنصر مجاز قرار بگیرد. نویسهٔ تزئینی ناوبری موبایل فروشنده حذف
شده است تا ارزیابی کنتراست به‌خاطر glyph نامعین متوقف نشود؛ برچسب فارسی هر مقصد همچنان
نام و کار آن را کامل می‌گوید. بازهٔ گزارش فروش نیز section معنایی است و icon صریح PWA از
درخواست خودکار favicon ناموجود جلوگیری می‌کند. fixture پرداخت projection انجام سفارش را
با upsert قطعی می‌سازد تا ترتیب پردازش worker باعث تعارض کلید یکتا نشود.

رابط [[ساخت] ساخت رابط محتوای فروش فروشنده](https://github.com/Mahaan-Amr/sevomart/issues/154)
با commit `81c9b8e3f4712170306afbdbfa09d90eb6cfae93` در `main` ادغام شده و سفر واقعی seller
content اکنون برای ماتریس و اجرای نهایی دروازه در دسترس است.
