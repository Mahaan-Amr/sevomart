...
برای Issue «بررسی درخواست فروشندگی توسط عامل پلتفرم»، مالک `identity-access` علاوه بر
fragment خودش، scheme ازپیش‌تصمیم‌گرفته‌شده `platformAgentSession` را در composer مرکزی
OpenAPI ثبت می‌کند. این استثنای محدود در همان Issue اعلام شده و مالکیت سایر بخش‌های
composer را تغییر نمی‌دهد.
...
| هویت و دسترسی      | `PlatformPermission*.v1`، `SensitiveAccess*.v1` و `EmergencyAccess*.v1`؛ command/query/errorهای متناظر در `@sevo/contracts/identity-access/v1`                                                                   | اداره مجوز مسئولیت و دسترسی حساس/اضطراری                                    | resolver نشست عامل، فضای کار پلتفرم و بازبین دسترسی؛ لغو، درخواست بعدی و نشست اضطراری را فوراً بی‌اثر می‌کند | sync/event           | بررسی و لغو strong؛ اعلان eventual                         | event فقط شناسه، scope کدشده و انقضا؛ audit دلیل داخلی دارد اما مقدار داده حساس ندارد | grant، audit و outbox مالک اتمیک‌اند؛ تغییر حساس داخل transaction مجوز را دوباره می‌سنجد     | `contract-blocks` فضای کار پلتفرم و producerهای دسترسی                                     |
...
| کالا               | `ProductPublished.v2`، `ProductUnpublished.v1` و `VariantPriceChanged.v1`                                                                                                                                        | ثبت انتشار، توقف انتشار یا تغییر offer                                      | فیدها و دنبال‌کردن؛ invalidation سبد/checkout                                                                | event                | eventual و نسخه‌محور                                       | ندارد                                                                                 | همراه تغییر کالا در outbox                                                                   | `contract-blocks` schema رخداد؛ `integration-blocks` projection فید                        |
...
| سفارش              | `BuyerOrderRead.v1`، `SellerActionableOrderRead.v1` و `StoreBuyerOrderHistoryRead.v1`                                                                                                                            | queryهای سفارش                                                              | پیگیری خریدار، فضای کار فروشنده، انجام سفارش و پرونده اختلاف آینده                                           | sync                 | strong                                                     | تاریخچه فقط همان فروشگاه و خریدار زمینه‌ای است؛ reveal دوباره ممیزی می‌شود            | read-only؛ cursor تاریخچه به فروشگاه و سفارش زمینه‌ای bind است؛ reveal audit مستقل دارد      | قرارداد producer-owned در Spec checkout؛ تاریخچه `contract-blocks` رابط Issue 150          |
| سفارش              | `OrderPurchaseExperienceEligibilityRead.v1`                                                                                                                                                                      | خواندن معتبر قلم سفارش                                                      | محتوا برای تأیید خرید                                                                                        | sync                 | strong                                                     | تصمیم مثبت شناسه‌های لازم را دارد؛ تصمیم منفی داده‌ای ندارد                           | read-only؛ فقط قلم همان خریدار در سفارش `PAID` مثبت است                                      | `contract-blocks` محتوا؛ `integration-blocks` PostgreSQL واقعی                             |
...
| سفارش              | `OrderCancellationPendingRefund.v1` و `OrderCancelled.v1`                                                                                                                                                        | ثبت انتظار بازپرداخت و لغو نهایی                                            | رابط پیگیری، انجام سفارش، اعلان‌ها و گزارش عملیاتی                                                           | event                | eventual؛ state عملیاتی در transaction strong              | ندارد؛ دلیل و شناسه مدرک ممنوع                                                        | پرداخت، سفارش، انجام سفارش و موجودی از seamهای producer در یک transaction هماهنگ می‌شوند     | نتیجه معتبر `DirectRefund*.v1`؛ `contract-blocks` رابط‌های پیگیری                          |
| انجام سفارش        | `FulfillmentTimeline.v1` و `FulfillmentOrderSnapshot.v1`                                                                                                                                                         | state machine و queryهای انجام سفارش                                        | رابط پیگیری خریدار و فروشنده؛ پرونده اختلاف برای پنجره اقدام                                                 | sync                 | strong؛ timeline و snapshot از یک aggregate خوانده می‌شوند | snapshot فقط شناسه‌های خریدار، فروشگاه و سفارش و زمان‌های ارسال/تحویل دارد            | تغییر وضعیت، timeline، idempotency و outbox مالک اتمیک‌اند؛ queryها read-only هستند          | مصرف `OrderBecameActionable.v1`؛ `contract-blocks` رابط‌ها و اختلاف                        |
| انجام سفارش        | `FulfillmentAdvanced.v1`                                                                                                                                                                                         | هر گذار معتبر انجام سفارش                                                   | اعلان‌ها، گزارش عملیاتی و projectionهای آینده                                                                | event                | eventual؛ تحویل حداقل یک‌بار                               | ندارد؛ روش ارسال و کد رهگیری در event ممنوع است                                       | همراه تغییر aggregate و timeline در outbox                                                   | `contract-blocks` schema رخداد؛ consumerها پیش از اتصال `integration-blocks`               |
...
| پرداخت             | `DirectRefund.v1` و `RecordDirectRefundResult.v1`                                                                                                                                                                | آغاز لغو توسط فروشنده؛ verify نتیجه امضاشده provider و ثبت ممیزی            | سفارش، انجام سفارش، موجودی و رابط فروشنده                                                                    | sync                 | verify بیرون transaction؛ اعمال نتیجه و لغو strong         | reason و evidence فقط در جدول ممیزی مالک؛ پاسخ عمومی فاقد هر دو                       | نتیجه معتبر، audit، سه producer همکار و outbox در یک transaction؛ تماس provider بیرون آن است | `DirectPaymentAttempt.v1` و `DirectPaymentProvider.v1`؛ `integration-blocks` adapter واقعی |
| پرداخت             | `DirectRefundPending.v1`، `DirectRefundConfirmed.v1` و `DirectRefundFailed.v1`                                                                                                                                   | ثبت هر گذار بازپرداخت مستقیم                                                | سفارش، اعلان‌ها، گزارش عملیاتی و projection پیگیری                                                           | event                | eventual؛ ثبت نتیجه idempotent                             | ندارد؛ reason، evidence و metadata خام provider ممنوع                                 | همراه وضعیت و audit بازپرداخت در outbox؛ لغو نهایی participantهای دیگر همان transaction است  | `contract-blocks` schema رخداد و رابط‌ها                                                   |
