...
## اسکن دسترس‌پذیری نسخه نمایشی

در [ثبت baselineهای بصری و evidence pack نسخه نمایشی](https://github.com/Mahaan-Amr/sevomart/issues/166)،
`@axe-core/playwright` نسخه دقیق `4.13.0` با مجوز `MPL-2.0` به وابستگی‌های توسعه اضافه می‌شود.
نگهدارنده Deque و مخزن رسمی `dequelabs/axe-core-npm` است؛ نسخه و مجوز در ۲۰۲۶-۰۹-۰۳ از registry بررسی شد.
این ابزار برای الزام اسکن serious/critical تصمیم پذیرش لازم است؛ سنجش هندسه و کنتراست موجود جای اسکن قواعد دسترس‌پذیری را نمی‌گیرد.
اسکن فقط در مرورگر آزمون محلی اجرا می‌شود و سرویس خارجی یا dependency runtime محصول اضافه نمی‌کند.
گزارش تحویلی فقط شناسه قاعده، شدت و تعداد موارد را نگه می‌دارد؛ HTML، متن، selector و URL نتیجهٔ خام به artifact راه نمی‌یابند.
نسخه دقیق، lockfile و سیاست supply-chain موجود برقرارند. مرور دستی صفحه‌کلید، متن فارسی و baseline همچنان مستقل و اجباری است.
...
| Sharp و libvips                                   | تشخیص واقعی و قابل‌خواندن بودن لوگو و جلد پیش از ذخیره     | Sharp فعال؛ Apache-2.0، libvips فعال؛ LGPL-2.1-or-later | پردازش بایت ورودی و binary بومی؛ نسخه دقیق، build مجاز صریح، سقف ۲۴ میلیون پیکسل، توقف روی هشدار decode، تطبیق format با MIME، audit و به‌روزرسانی امنیتی |
| Fastify multipart                                 | دریافت stream محدود فایل بدون سربار Base64                 | فعال؛ MIT                                               | ورودی شبکه؛ سقف ۱۰ MB، یک فایل و یک field، decode کامل و rate limit                                                                                      |
| MinIO SDK و MinIO Server                          | object storage خصوصی اصل و مشتق رسانه                      | فعال؛ Apache-2.0 / AGPL-3.0                             | credential فقط runtime، bucket خصوصی، bind محلی و تحویل فقط از API                                                                                       |
