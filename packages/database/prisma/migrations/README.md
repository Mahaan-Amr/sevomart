...
Issue 126 additively creates the platform-owned managed-data target fingerprint and
versioned demo-manifest receipt. Existing product data is not read or changed, no
compatibility window is required, and deployment corrections use a forward
migration. Both supported startup paths continue to use the same `prisma migrate
deploy` history; their smoke evidence is recorded in the delivery note for the
Issue.
