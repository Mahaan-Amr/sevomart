...
type StorefrontResult =
  | { state: "ready"; store: PublicStore }
  | { state: "not-found" }
  | { state: "error" };

async function readPublishedStore(slug: string): Promise<StorefrontResult> {
  try {
    const response = await fetch(`${API_BASE_URL}/v1/stores/${encodeURIComponent(slug)}`, {
      cache: "no-store",
...
export default async function StorefrontPage({
  ...
  if (process.env.SEVO_RUNTIME_ENV === "test") {
    if (slug === "test-loading") {
      return <StorefrontPageFrame><LoadingStorefront /></StorefrontPageFrame>;
    }
    if (slug === "test-error") {
      return <StorefrontPageFrame><ErrorStorefront retryHref={`/s/${slug}`} /></StorefrontPageFrame>;
    }
  }
  ...
  return <StorefrontPageFrame>{result.state === "ready" ? <ReadyStorefront store={result.store} /> : <ErrorStorefront retryHref={`/s/${slug}`} />}</StorefrontPageFrame>;
}
